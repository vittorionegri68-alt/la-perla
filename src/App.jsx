...
// ── Design Tokens — Digital Nomad Friendly #9 ─────────────────────────────────
const C = {
  bg:      "#F9FAFB",
  bg2:     "#F3F4F6",
  bg3:     "#E5E7EB",
...
function Tag({ children, color = C.blue }) {
  return (
    <span style={{
      ...
      background: color === C.blue ? "rgba(59,130,246,0.1)" : "rgba(16,185,129,0.1)",
      color, fontSize: "0.72rem", fontWeight: 600, letterSpacing: "0.04em",
...
function Nav() {
  ...
  const links = [["Appartamento","#appartamento"],["Esperienze","#esperienze"],["Posizione","#posizione"],["Recensioni","#recensioni"]];

  return (
    <nav style={{
      position: "fixed", top: 0, left: 0, right: 0, zIndex: 200,
      background: scrolled ? "rgba(249,250,251,0.97)" : "rgba(249,250,251,0.9)",
      ...
      <div style={{ maxWidth: 1200, margin: "0 auto", padding: "0 1.5rem", height: 64, display: "flex", alignItems: "center", justifyContent: "space-between" }}>
        <div style={{ display: "flex", alignItems: "center", gap: "0.5rem" }}>
          ...
          <div style={{ fontFamily: "'Inter','DM Sans',sans-serif", fontSize: "0.95rem", fontWeight: 700, color: C.text, letterSpacing: "-0.02em" }}>
            La Perla del Mare
          </div>
        </div>
        ...
        <div style={{ display: "flex", gap: "2rem", alignItems: "center" }} className="desk-nav">
          {links.map(([l, h]) => (
            <a key={l} href={h} style={{ color: C.textMid, textDecoration: "none", fontSize: "0.82rem", fontWeight: 500, fontFamily: "'Inter','DM Sans',sans-serif", transition: "color 0.2s" }}
          ...
          <div style={{ display: "flex", gap: "0.25rem", borderLeft: `1px solid ${C.border}`, paddingLeft: "1rem" }}>
            <a href={config.langIT} style={{ fontSize: "0.72rem", fontWeight: 700, color: C.blue, textDecoration: "none", fontFamily: "'Inter',sans-serif", padding: "0.2rem 0.35rem", borderRadius: 4, background: "rgba(59,130,246,0.1)" }}>IT</a>
            <a href={config.langEN} style={{ fontSize: "0.72rem", fontWeight: 500, color: C.textSoft, textDecoration: "none", fontFamily: "'Inter',sans-serif", padding: "0.2rem 0.35rem", borderRadius: 4, transition: "color 0.2s" }}
              onMouseEnter={e => e.target.style.color = C.blue}
              onMouseLeave={e => e.target.style.color = C.textSoft}>EN</a>
          </div>
          <a href={config.airbnbUrl} target="_blank" rel="noopener noreferrer"
            style={{ background: C.blue, color: "#fff", padding: "0.5rem 1.25rem", borderRadius: 8, fontSize: "0.8rem", fontWeight: 600, textDecoration: "none", fontFamily: "'Inter','DM Sans',sans-serif", transition: "all 0.2s", boxShadow: `0 1px 3px rgba(59,130,246,0.3)` }}
        ...
        <div style={{ background: C.cardBg, borderTop: `1px solid ${C.border}`, padding: "1rem 1.5rem 1.5rem" }}>
          {links.map(([l, h]) => (
            <a key={l} href={h} onClick={() => setOpen(false)}
              style={{ display: "block", color: C.textMid, textDecoration: "none", padding: "0.65rem 0", fontFamily: "'Inter',sans-serif", fontSize: "0.95rem", borderBottom: `1px solid ${C.border}` }}>{l}</a>
          ))}
          <div style={{ display: "flex", gap: "0.5rem", margin: "1rem 0 0.75rem" }}>
            <a href={config.langIT} style={{ fontSize: "0.75rem", fontWeight: 700, color: C.blue, textDecoration: "none", padding: "0.3rem 0.75rem", borderRadius: 6, background: "rgba(59,130,246,0.1)" }}>IT</a>
            <a href={config.langEN} style={{ fontSize: "0.75rem", fontWeight: 500, color: C.textSoft, textDecoration: "none", padding: "0.3rem 0.75rem", borderRadius: 6, background: C.bg2 }}>EN</a>
          </div>
          <a href={config.airbnbUrl} target="_blank" rel="noopener noreferrer"
            style={{ display: "inline-block", background: C.blue, color: "#fff", padding: "0.7rem 1.5rem", borderRadius: 8, fontSize: "0.82rem", fontWeight: 600, textDecoration: "none", fontFamily: "'Inter',sans-serif" }}>
            Prenota su Airbnb
          </a>
        </div>
      )}
      ...
      <div style={{ position: "relative", zIndex: 2, maxWidth: 1200, margin: "0 auto", padding: "0 1.5rem", minHeight: "100vh", display: "flex", alignItems: "center", paddingTop: "5rem" }}>
        <div style={{ maxWidth: 680 }}>
          <div style={{ opacity: loaded?1:0, transform: loaded?"translateY(0)":"translateY(16px)", transition: "all 0.6s ease 0.1s", display: "flex", gap: "0.5rem", flexWrap: "wrap", marginBottom: "1.5rem" }}>
            ...
            <Tag color={C.blue}>400m dal Mare</Tag>
            <Tag color={C.blue}>Garage Privato</Tag>
          </div>

          <h1 style={{ opacity: loaded?1:0, transform: loaded?"translateY(0)":"translateY(24px)", transition: "all 0.8s ease 0.2s", fontFamily: "'Inter','DM Sans',sans-serif", fontSize: "clamp(2.4rem,5.5vw,4.5rem)", fontWeight: 800, color: "#fff", lineHeight: 1.1, letterSpacing: "-0.03em", marginBottom: "1.25rem" }}>
            Il mare a portata<br />
            <span style={{ color: C.blue }}>di bicicletta.</span>
          </h1>

          <p style={{ opacity: loaded?1:0, transform: loaded?"translateY(0)":"translateY(20px)", transition: "all 0.8s ease 0.35s", fontFamily: "'Inter','DM Sans',sans-serif", fontSize: "clamp(0.95rem,1.8vw,1.1rem)", color: "rgba(255,255,255,0.75)", lineHeight: 1.8, maxWidth: 500, marginBottom: "2rem" }}>
            Appartamento luminoso con garage privato e biciclette incluse, a 400 metri dalla spiaggia di Cesenatico. Perfetto per famiglie, gruppi di amici e smart workers.
          </p>
          ...
          <div style={{ opacity: loaded?1:0, transform: loaded?"translateY(0)":"translateY(20px)", transition: "all 0.8s ease 0.48s", display: "flex", gap: "0.75rem", flexWrap: "wrap" }}>
            <a href={config.airbnbUrl} target="_blank" rel="noopener noreferrer"
              style={{ background: C.blue, color: "#fff", padding: "0.9rem 2rem", borderRadius: 10, fontSize: "0.88rem", fontWeight: 700, textDecoration: "none", fontFamily: "'Inter',sans-serif", boxShadow: "0 4px 14px rgba(59,130,246,0.4)", transition: "all 0.25s", display: "flex", alignItems: "center", gap: "0.4rem" }}
              onMouseEnter={e => { e.currentTarget.style.background = "#2563EB"; e.currentTarget.style.transform = "translateY(-2px)"; }}
              onMouseLeave={e => { e.currentTarget.style.background = C.blue; e.currentTarget.style.transform = "translateY(0)"; }}>
              Prenota su Airbnb ↗
            </a>
            <a href="#appartamento"
              style={{ background: "rgba(255,255,255,0.1)", color: "#fff", padding: "0.9rem 1.75rem", borderRadius: 10, fontSize: "0.88rem", fontWeight: 600, textDecoration: "none", fontFamily: "'Inter',sans-serif", border: "1px solid rgba(255,255,255,0.2)", transition: "all 0.25s" }}
          ...
          <div style={{ opacity: loaded?1:0, transition: "opacity 0.8s ease 0.65s", display: "flex", gap: "2rem", marginTop: "3rem", paddingTop: "2rem", borderTop: "1px solid rgba(255,255,255,0.1)", flexWrap: "wrap" }}>
            {[["6","Ospiti"],["2","Camere"],["27","Recensioni"],["4,96","Rating"]].map(([v,l]) => (
              <div key={l}>
...
function Apartment() {
  ...
  const amenities = [
    { icon: "🚗", title: "Garage privato", desc: "Box auto sotto casa + biciclette incluse — rarissimo in zona" },
    { icon: "🛗", title: "Ascensore", desc: "Accesso senza barriere dal garage al piano" },
    { icon: "📶", title: "WiFi fibra", desc: "Connessione veloce, workspace dedicato" },
    { icon: "❄️", title: "A/C in ogni stanza", desc: "Split in ogni camera e nel living" },
    { icon: "👨‍🍳", title: "Cucina completa", desc: "Forno, lavastoviglie, macchinetta, tutto il necessario" },
    { icon: "🌅", title: "2 Balconi", desc: "Esposti a sud, con tavolino e sedie — perfetti per il caffe" },
  ];

  return (
    <section id="appartamento" style={{ background: C.bg, padding: "6rem 1.5rem" }}>
      <div style={{ maxWidth: 1200, margin: "0 auto" }}>
        <Reveal>
          <div style={{ display: "flex", alignItems: "center", gap: "0.5rem", marginBottom: "0.5rem" }}>
            <div style={{ width: 3, height: 18, background: C.blue, borderRadius: 2 }} />
            <span style={{ fontFamily: "'Inter',sans-serif", fontSize: "0.78rem", fontWeight: 600, color: C.blue, textTransform: "uppercase", letterSpacing: "0.1em" }}>L'Appartamento</span>
          </div>
          <h2 style={{ fontFamily: "'Inter',sans-serif", fontSize: "clamp(1.8rem,4vw,3rem)", fontWeight: 800, color: C.text, letterSpacing: "-0.03em", marginBottom: "0.5rem" }}>
            Spazio, luce e tutto il comfort
          </h2>
          <p style={{ fontFamily: "'Inter',sans-serif", fontSize: "1rem", color: C.textMid, lineHeight: 1.75, maxWidth: 580, marginBottom: "3rem" }}>
            Bilocale ristrutturato, esposto a sud, con ampi spazi comuni. Il box privato con biciclette incluse e una rarità nella zona.
          </p>
        </Reveal>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1.1fr 0.9fr", gap: "3rem", alignItems: "start" }} className="apt-grid">
          <Reveal>
            <div>
              <div style={{ borderRadius: 12, overflow: "hidden", boxShadow: C.shadowLg, marginBottom: "0.5rem", background: C.bg3 }}>
                <img src={ROOMS[active]} alt="La Perla del Mare" loading="lazy"
          ...
          <Reveal delay={100}>
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "0.75rem", marginBottom: "1.75rem" }}>
              {amenities.map(({ icon, title, desc }) => (
                <div key={title} style={{ background: C.cardBg, padding: "1rem", borderRadius: 10, border: `1px solid ${C.border}`, transition: "all 0.2s", cursor: "default" }}
            ...
            <div style={{ background: "linear-gradient(135deg, rgba(59,130,246,0.06) 0%, rgba(16,185,129,0.06) 100%)", border: `1px solid rgba(59,130,246,0.15)`, borderRadius: 10, padding: "1.25rem", marginBottom: "1.5rem" }}>
              ...
              <div style={{ display: "flex", gap: "2rem" }}>
                ...
                <div><div style={{ fontFamily: "'Inter',sans-serif", fontSize: "1.1rem", fontWeight: 700, color: C.text }}>Max 6</div><div style={{ fontFamily: "'Inter',sans-serif", fontSize: "0.7rem", color: C.textSoft }}>Ospiti</div></div>
              </div>
            </div>
...
function Experience() {
  const items = [
    { img: LIFESTYLE[0], tag: "Mare", tagColor: C.blue, title: "400 metri dalla spiaggia", desc: "Prendi le biciclette dal garage e in pochi minuti sei in riva al mare. Il lungomare di Cesenatico e uno dei piu belli della Riviera." },
    { img: LIFESTYLE[1], tag: "Smart Work", tagColor: C.green, title: "WiFi fibra e workspace", desc: "Connessione veloce e scrivania dedicata. Lavora la mattina, stai in spiaggia il pomeriggio. Il work-life balance ideale." },
    { img: LIFESTYLE[2], tag: "Natura", tagColor: C.green, title: "Parco di Levante a 4 minuti", desc: "40 ettari di prato, pineta e laghetti con fauna. Perfetto per una passeggiata mattutina o una corsa tra i pini." },
    { img: LIFESTYLE[3], tag: "Territorio", tagColor: C.blue, title: "La Romagna a portata di auto", desc: "Bertinoro a 25 min, Ravenna UNESCO a 50 min, San Marino a 35 min. Una base ideale per esplorare la regione." },
  ];

  return (
    <section id="esperienze" style={{ background: C.bg2, padding: "6rem 1.5rem" }}>
      <div style={{ maxWidth: 1200, margin: "0 auto" }}>
        <Reveal>
          <div style={{ display: "flex", alignItems: "center", gap: "0.5rem", marginBottom: "0.5rem" }}>
            <div style={{ width: 3, height: 18, background: C.green, borderRadius: 2 }} />
            <span style={{ fontFamily: "'Inter',sans-serif", fontSize: "0.78rem", fontWeight: 600, color: C.green, textTransform: "uppercase", letterSpacing: "0.1em" }}>Esperienze</span>
          </div>
          <h2 style={{ fontFamily: "'Inter',sans-serif", fontSize: "clamp(1.8rem,4vw,3rem)", fontWeight: 800, color: C.text, letterSpacing: "-0.03em", marginBottom: "0.5rem" }}>
            Mare, natura e smart working.<br />
            <span style={{ color: C.blue }}>Tutto in un posto.</span>
          </h2>
          <p style={{ fontFamily: "'Inter',sans-serif", fontSize: "1rem", color: C.textMid, lineHeight: 1.75, maxWidth: 540, marginBottom: "3rem" }}>
            Cesenatico e la destinazione perfetta per chi vuole combinare relax, natura e produttivita.
          </p>
        </Reveal>
            ...
            <Reveal key={title} delay={i * 60}>
              <div style={{ background: C.cardBg, borderRadius: 12, overflow: "hidden", border: `1px solid ${C.border}`, transition: "all 0.25s", boxShadow: C.shadow }}
                ...
                <div style={{ overflow: "hidden", height: 200 }}>
                  <img src={img} alt={title} loading="lazy" style={{ width: "100%", height: "100%", objectFit: "cover", transition: "transform 0.5s ease" }}
        ...
        <Reveal delay={120}>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(5,1fr)", gap: "1px", background: C.border, marginTop: "3rem", borderRadius: 12, overflow: "hidden" }} className="dist-grid">
            {[["400 m","dalla spiaggia"],["15 min","da Cesena"],["25 min","da Bertinoro"],["35 min","da San Marino"],["50 min","da Ravenna"]].map(([v,l]) => (
...
function HowItWorks() {
  const steps = [
    { n: "01", t: "Scegli le date", b: "Controlla la disponibilita su Airbnb. Prenotazione sicura con pagamento protetto dalla piattaforma.", color: C.blue },
    { n: "02", t: "Conferma istantanea", b: "Ricevi immediatamente la conferma, le istruzioni di accesso e i consigli locali di Cesare.", color: C.blue },
    { n: "03", t: "Self check-in", b: "Smartlock — arrivi quando vuoi, senza aspettare nessuno. Le chiavi digitali ti aspettano.", color: C.green },
    { n: "04", t: "Goditi Cesenatico", b: "Biciclette nel garage, mare a 400 m, cucina completa. La tua vacanza inizia subito.", color: C.green },
  ];
    ...
    <section style={{ background: C.bg, padding: "6rem 1.5rem" }}>
      <div style={{ maxWidth: 1200, margin: "0 auto" }}>
        <Reveal>
          <div style={{ display: "flex", alignItems: "center", gap: "0.5rem", marginBottom: "0.5rem" }}>
            <div style={{ width: 3, height: 18, background: C.blue, borderRadius: 2 }} />
            <span style={{ fontFamily: "'Inter',sans-serif", fontSize: "0.78rem", fontWeight: 600, color: C.blue, textTransform: "uppercase", letterSpacing: "0.1em" }}>Come funziona</span>
          </div>
          <h2 style={{ fontFamily: "'Inter',sans-serif", fontSize: "clamp(1.8rem,4vw,3rem)", fontWeight: 800, color: C.text, letterSpacing: "-0.03em", marginBottom: "3rem" }}>
            Prenotare e semplice<br />come dovrebbe essere.
          </h2>
        </Reveal>
...
function Testimonials() {
  const reviews = [
    { q: "L'alloggio si presenta come nelle foto, accogliente, pulito e di ogni comfort. Posizione ottima, vicino al mare e ai servizi. I consigli su cosa fare in zona sono stati molto utili.", name: "Nicole", origin: "Parigi", stars: 5 },
    { q: "Trovato all'ultimo minuto. La casa e pulita e lo spazio distribuito in maniera perfetta. Il garage privato e le biciclette sono stati un plus enorme.", name: "Emanuele", origin: "Roma", stars: 5 },
    { q: "L'appartamento e carino e tenuto bene, ha tutte le caratteristiche che si vorrebbero. Il riscaldamento funziona benissimo, abbiamo trascorso un capodanno felice.", name: "Francesco", origin: "Como", stars: 5 },
    { q: "Spazioso, luminoso e dotato di tutti i comfort. Posizione eccellente, vicinissimi al mare. Torneremo sicuramente.", name: "Marco & Giulia", origin: "Milano", stars: 5 },
  ];

  return (
    <section id="recensioni" style={{ background: C.bg2, padding: "6rem 1.5rem" }}>
      <div style={{ maxWidth: 1200, margin: "0 auto" }}>
        <Reveal>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-end", marginBottom: "2.5rem", flexWrap: "wrap", gap: "1rem" }}>
            <div>
              <div style={{ display: "flex", alignItems: "center", gap: "0.5rem", marginBottom: "0.5rem" }}>
                <div style={{ width: 3, height: 18, background: C.green, borderRadius: 2 }} />
                <span style={{ fontFamily: "'Inter',sans-serif", fontSize: "0.78rem", fontWeight: 600, color: C.green, textTransform: "uppercase", letterSpacing: "0.1em" }}>Recensioni</span>
              </div>
              <h2 style={{ fontFamily: "'Inter',sans-serif", fontSize: "clamp(1.8rem,4vw,3rem)", fontWeight: 800, color: C.text, letterSpacing: "-0.03em" }}>
                Cosa dicono gli ospiti.
              </h2>
            </div>
            <div style={{ display: "flex", gap: "1.5rem" }}>
              {[["★ 4,96","Rating"],["27","Recensioni"],["100%","5 stelle"]].map(([v,l]) => (
            ...
            <Reveal key={name}>
              <div style={{ background: C.cardBg, padding: "1.75rem", borderRadius: 12, border: `1px solid ${C.border}`, transition: "all 0.2s", boxShadow: C.shadow }}
                ...
                <div style={{ display: "flex", gap: "2px", marginBottom: "1rem" }}>
                  {Array(stars).fill("★").map((s,i) => <span key={i} style={{ color: "#FBBF24", fontSize: "0.85rem" }}>{s}</span>)}
                </div>
                ...
                <div style={{ display: "flex", alignItems: "center", gap: "0.6rem", borderTop: `1px solid ${C.border}`, paddingTop: "1rem" }}>
                  <div style={{ width: 32, height: 32, borderRadius: "50%", background: `linear-gradient(135deg, ${C.blue}, ${C.green})`, display: "flex", alignItems: "center", justifyContent: "center", color: "#fff", fontFamily: "'Inter',sans-serif", fontSize: "0.75rem", fontWeight: 700, flexShrink: 0 }}>
                    {name.charAt(0)}
                  </div>
                  <div>
...
function Location() {
  const places = [
    { icon: "🏖️", name: "Spiaggia e mare", dist: "400 m", tag: "A piedi" },
    { icon: "🛒", name: "Negozi e ristoranti", dist: "5 min", tag: "A piedi" },
    { icon: "🌳", name: "Parco di Levante (40 ha)", dist: "4 min", tag: "A piedi" },
    { icon: "🎡", name: "Parco Kinderland", dist: "2 min", tag: "A piedi" },
    { icon: "⛵", name: "Porto canale Leonardo da Vinci", dist: "10 min", tag: "A piedi" },
    { icon: "🚗", name: "Cesena", dist: "15 min", tag: "In auto" },
    { icon: "🏖️", name: "Rimini", dist: "30 min", tag: "In auto" },
    { icon: "🏰", name: "Repubblica di San Marino", dist: "35 min", tag: "In auto" },
  ];

  return (
    <section id="posizione" style={{ background: C.bg, padding: "6rem 1.5rem" }}>
      <div style={{ maxWidth: 1200, margin: "0 auto" }}>
        <Reveal>
          <div style={{ display: "flex", alignItems: "center", gap: "0.5rem", marginBottom: "0.5rem" }}>
            <div style={{ width: 3, height: 18, background: C.blue, borderRadius: 2 }} />
            <span style={{ fontFamily: "'Inter',sans-serif", fontSize: "0.78rem", fontWeight: 600, color: C.blue, textTransform: "uppercase", letterSpacing: "0.1em" }}>Posizione</span>
          </div>
          <h2 style={{ fontFamily: "'Inter',sans-serif", fontSize: "clamp(1.8rem,4vw,3rem)", fontWeight: 800, color: C.text, letterSpacing: "-0.03em", marginBottom: "3rem" }}>
            Nel cuore di Cesenatico.<br />
            <span style={{ color: C.blue }}>Vicino a tutto.</span>
          </h2>
        </Reveal>
          ...
          <Reveal delay={100}>
            <div style={{ display: "flex", flexDirection: "column", gap: "1rem" }}>
              <div style={{ background: "linear-gradient(135deg, rgba(59,130,246,0.06), rgba(16,185,129,0.06))", border: `1px solid rgba(59,130,246,0.15)`, borderRadius: 12, padding: "1.5rem" }}>
                <div style={{ fontFamily: "'Inter',sans-serif", fontSize: "0.72rem", fontWeight: 600, color: C.blue, textTransform: "uppercase", letterSpacing: "0.08em", marginBottom: "0.75rem" }}>Il Quartiere</div>
                <p style={{ fontFamily: "'Inter',sans-serif", fontSize: "0.88rem", color: C.textMid, lineHeight: 1.8, margin: 0 }}>
                  Cesenatico e una delle localita balneari piu amate della Riviera. Vicino al porto canale progettato da Leonardo da Vinci, con ristoranti, negozi e servizi a portata di passeggiata.
                </p>
              </div>
              <div style={{ background: C.cardBg, border: `1px solid ${C.border}`, borderRadius: 12, padding: "1.5rem" }}>
                <div style={{ fontFamily: "'Inter',sans-serif", fontSize: "0.72rem", fontWeight: 600, color: C.textSoft, textTransform: "uppercase", letterSpacing: "0.08em", marginBottom: "0.75rem" }}>Indirizzo</div>
                <div style={{ fontFamily: "'Inter',sans-serif", fontSize: "0.88rem", color: C.text, lineHeight: 1.7 }}>{contactInfo.address}</div>
                <div style={{ fontFamily: "'Inter',sans-serif", fontSize: "0.78rem", color: C.textSoft, marginTop: "0.5rem" }}>Posizione esatta fornita dopo la prenotazione.</div>
              </div>
              <div style={{ background: C.cardBg, border: `1px solid ${C.border}`, borderRadius: 12, padding: "1.5rem" }}>
                ...
                <div style={{ fontFamily: "'Inter',sans-serif", fontSize: "0.78rem", color: C.textSoft, marginTop: "0.25rem" }}>100% risposta · Risponde entro 1 ora</div>
              </div>
            </div>
          </Reveal>
        </div>
      ...
      <div style={{ maxWidth: 800, margin: "0 auto", textAlign: "center", position: "relative", zIndex: 1 }}>
        <Reveal>
          <div style={{ display: "flex", justifyContent: "center", gap: "0.5rem", marginBottom: "1.5rem" }}>
            <Tag color={C.green}>★ 4,96 Guest Favourite</Tag>
            <Tag color={C.blue}>27 Recensioni</Tag>
          </div>
          <h2 style={{ fontFamily: "'Inter',sans-serif", fontSize: "clamp(2rem,5vw,3.8rem)", fontWeight: 800, color: "#fff", letterSpacing: "-0.03em", lineHeight: 1.1, marginBottom: "1.25rem" }}>
            La tua estate a Cesenatico<br />
            <span style={{ color: C.blue }}>inizia qui.</span>
          </h2>
          <p style={{ fontFamily: "'Inter',sans-serif", fontSize: "1rem", color: "rgba(255,255,255,0.65)", lineHeight: 1.8, maxWidth: 520, margin: "0 auto 2.5rem" }}>
            Le date estive si esauriscono rapidamente. Controlla la disponibilita su Airbnb e prenota il tuo soggiorno al mare.
          </p>
          <a href={config.airbnbUrl} target="_blank" rel="noopener noreferrer"
            ...
            onMouseEnter={e => { e.currentTarget.style.background = "#2563EB"; e.currentTarget.style.transform = "translateY(-3px)"; e.currentTarget.style.boxShadow = "0 12px 30px rgba(59,130,246,0.45)"; }}
            onMouseLeave={e => { e.currentTarget.style.background = C.blue; e.currentTarget.style.transform = "translateY(0)"; e.currentTarget.style.boxShadow = "0 8px 24px rgba(59,130,246,0.35)"; }}>
          ...
          <div style={{ marginTop: "1.25rem", fontFamily: "'Inter',sans-serif", fontSize: "0.75rem", color: "rgba(255,255,255,0.4)", letterSpacing: "0.06em" }}>
            Prenotazione sicura · Conferma immediata · Rating 4,96 stelle
          </div>
        </Reveal>
      </div>
    </section>
      ...
      <div style={{ maxWidth: 1200, margin: "0 auto" }}>
        <div style={{ display: "grid", gridTemplateColumns: "2fr 1fr 1fr", gap: "3rem", marginBottom: "3rem" }} className="footer-grid">
          <div>
            ...
            <div style={{ fontFamily: "'Inter',sans-serif", fontSize: "0.78rem", color: "rgba(255,255,255,0.4)", marginBottom: "1rem" }}>Appartamento vacanze · Cesenatico · IT</div>
            ...
            <div style={{ marginTop: "0.75rem" }}>
              <a href={`tel:${contactInfo.phone}`} style={{ display: "block", fontFamily: "'Inter',sans-serif", fontSize: "0.82rem", color: "rgba(255,255,255,0.55)", textDecoration: "none", marginBottom: "0.3rem", transition: "color 0.2s" }}
                onMouseEnter={e => e.target.style.color = C.blue} onMouseLeave={e => e.target.style.color = "rgba(255,255,255,0.55)"}>
                {contactInfo.phone}
              </a>
              <a href={`mailto:${contactInfo.email}`} style={{ fontFamily: "'Inter',sans-serif", fontSize: "0.82rem", color: "rgba(255,255,255,0.55)", textDecoration: "none", transition: "color 0.2s" }}
                onMouseEnter={e => e.target.style.color = C.blue} onMouseLeave={e => e.target.style.color = "rgba(255,255,255,0.55)"}>
                {contactInfo.email}
              </a>
            </div>
          ...
          <div>
            <div style={{ fontFamily: "'Inter',sans-serif", fontSize: "0.7rem", fontWeight: 600, color: "rgba(255,255,255,0.35)", textTransform: "uppercase", letterSpacing: "0.1em", marginBottom: "1rem" }}>Naviga</div>
            {[["Appartamento","#appartamento"],["Esperienze","#esperienze"],["Posizione","#posizione"],["Recensioni","#recensioni"]].map(([l,h]) => (
              <a key={l} href={h} style={{ display: "block", fontFamily: "'Inter',sans-serif", fontSize: "0.85rem", color: "rgba(255,255,255,0.55)", textDecoration: "none", marginBottom: "0.5rem", transition: "color 0.2s" }}
                onMouseEnter={e => e.target.style.color = C.blue} onMouseLeave={e => e.target.style.color = "rgba(255,255,255,0.55)"}>{l}</a>
            ))}
          </div>
          <div>
            <div style={{ fontFamily: "'Inter',sans-serif", fontSize: "0.7rem", fontWeight: 600, color: "rgba(255,255,255,0.35)", textTransform: "uppercase", letterSpacing: "0.1em", marginBottom: "1rem" }}>Prenota & Social</div>
            <a href={config.airbnbUrl} target="_blank" rel="noopener noreferrer"
            ...
            <a href={contactInfo.instagram} target="_blank" rel="noopener noreferrer"
              style={{ display: "block", fontFamily: "'Inter',sans-serif", fontSize: "0.82rem", color: "rgba(255,255,255,0.55)", textDecoration: "none", transition: "color 0.2s" }}
              onMouseEnter={e => e.target.style.color = C.blue} onMouseLeave={e => e.target.style.color = "rgba(255,255,255,0.55)"}>
              @luceacollection_ ↗
            </a>
            <div style={{ marginTop: "1.25rem", display: "flex", gap: "0.5rem" }}>
              <a href={config.langIT} style={{ fontSize: "0.72rem", fontWeight: 700, color: C.blue, textDecoration: "none", padding: "0.25rem 0.6rem", borderRadius: 6, background: "rgba(59,130,246,0.15)" }}>IT</a>
              <a href={config.langEN} style={{ fontSize: "0.72rem", fontWeight: 500, color: "rgba(255,255,255,0.4)", textDecoration: "none", padding: "0.25rem 0.6rem", borderRadius: 6, background: "rgba(255,255,255,0.06)", transition: "color 0.2s" }}
                onMouseEnter={e => e.target.style.color = C.blue} onMouseLeave={e => e.target.style.color = "rgba(255,255,255,0.4)"}>EN</a>
            </div>
          </div>
        </div>
        <div style={{ borderTop: "1px solid rgba(255,255,255,0.06)", paddingTop: "1.25rem", display: "flex", justifyContent: "space-between", flexWrap: "wrap", gap: "0.5rem" }}>
          ...
          <div style={{ fontFamily: "'Inter',sans-serif", fontSize: "0.72rem", color: "rgba(255,255,255,0.3)" }}>Prenotazione sicura tramite Airbnb</div>
        </div>
      </div>
